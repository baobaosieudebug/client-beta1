        ...
        <el-container v-if="Object.keys(curUser).length > 0">
          ...
          <el-footer>
            <div class="form-input">
              ...
              <button class="btn-send" @click="sendMenu">Send Menu</button>
            </div>
          </el-footer>
        </el-container>
...
export default {
  ...
  methods: {
    ...
    sendMenu() {
      const message = 'Bạn đang tìm kiếm điều gì?';
      socket.emit('sendMenu', {
        userID: this.curUser.userID,
        message
      });
    },
  ...
  mounted() {
    socket.on('user', (user) => {
      const userConnect = this.users.find((u) => u.userID === user.userID);
      if (!userConnect) {
        this.users.unshift(user);
      } else {
        userConnect.connected = true;
        userConnect.isRequest = true;
      }
      // this.curUserChatArray.push({ message, isUser: false });
    });
